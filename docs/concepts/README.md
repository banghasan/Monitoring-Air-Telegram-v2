# Concepts

Folder ini berisi konsep dan keputusan arsitektur yang menjadi dasar implementasi. Dokumen di sini menjawab “sistemnya akan bekerja seperti apa”.

## Dokumen konsep

- [Monitoring worker](./11-monitoring-worker.md)
- [Notifikasi perubahan](./12-monitoring-notifications.md)
- [Target Telegram](./13-telegram-notification-targets.md)
- [Structured logging](./14-structured-logging.md)

Dokumen detail dasar yang sudah ada di folder induk:

- [Scope dan command](../01-scope-and-commands.md)
- [Dokumentasi data](../data/README.md)
- [Rich Message](../05-telegram-rich-message.md)
- [Polling](../06-telegram-polling.md)
- [Environment](../07-environment-configuration.md)
- [Docker Compose](../08-docker-compose.md)
- [Owner/admin](../09-owner-admin-and-security.md)
- [Testing dan operasi](../10-testing-and-operations.md)

Jika sebuah keputusan belum cukup pasti, jangan memasukkannya sebagai konsep final. Catat di folder [`../issues/`](../issues/README.md).
