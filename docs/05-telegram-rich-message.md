...
Jika versi grammY yang dipakai belum memiliki type/method Rich Message, integrasi API harus diisolasi di adapter Telegram. Adapter tersebut wajib mengacu pada `telegram/api.md`; jangan menyebarkan request raw Bot API ke semua handler.
...
3. nama tampilan station dan link peta;
...
5. jika method edit tidak mendukung Rich Message atau edit gagal, bot mengirim Rich Message baru;
6. kegagalan edit dan alasan fallback dicatat di log, bukan ditampilkan sebagai stack trace ke pengguna.

Implementasi tidak boleh menganggap `editMessageText` otomatis menerima struktur Rich Message. Dukungan method edit harus diverifikasi terhadap API lokal dan versi grammY yang digunakan.
